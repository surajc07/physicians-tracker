...
class NewComment extends Component {
  ...
  handleSubmit(event) {
    ...
    this.setState({userId: cardObj.id,status: event.target[1].value, comment: event.target[0].value})
    console.log(this.state);
  ...
  render() {
    return (
      ...
      >
        ...
        <input id="comment" name="comment" type="text" onChange={() => this.handleCommentInput} value={this.state.value}  />

        <label htmlFor="status">Enter the status</label>
        <input id="status" name="status" type="text" onChange={() => this.handleStatusInput} value={this.state.value}  />
